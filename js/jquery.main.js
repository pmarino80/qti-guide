...
// json slide init
function initCreateSlide(){
	jQuery('.schedule-gallery').each(function(){
		var holder = jQuery(this);
		var carouselHolder = holder.find('.slideset');
		...
		jQuery.ajax({
			...
			success:function(response){
				...
				jQuery(data.slides).each(function(){
					...
					// create carouselSlide
					var slide = jQuery('<div class="slide"><ul class="schedule-list"></ul></div>');
					jQuery(dataSlide.week).each(function(){
						var day = this;
						var item = jQuery('<li></li>');
						var title = jQuery('<strong class="title"><a href="'+day.dayUrl+'">'+day.dayTitle+
						'</a></strong><time datetime="'+day.dataDate+'"><a href="'+day.dateUrl+'">'+day.dateTitle+'</a></time><ul></ul>');
						
						title.appendTo(item);
						jQuery(day.dateList).each(function(){
							var itemList = this;
							var itemBox = jQuery('<li><a href="'+itemList.timeUrl+'">'+itemList.time+'</a></li>');
							itemBox.appendTo(item.find('ul'));
						});
						item.appendTo(slide.find('.schedule-list'));
					});
					
					slide.appendTo(carouselHolder);
					initPlugin();
				});
			},
			error:function(){
		...
		// plugins init
		function initPlugin(){
			jQuery('div.carousel-nocycle').scrollGallery({
				mask: 'div.mask',
				slider: 'div.slideset',
				slides: 'div.slide',
				btnPrev: 'a.btn-prev',
...
function initScrollPage(){
	...
	var subWrap = jQuery('.smooth-wrap');
	var navOffset = subWrap.offset().top;
	
	subWrap.find('.smooth-list').each(function(){
		...
		items.each(function(){
			...
			item.bind(event, function(e){
				e.preventDefault();
				if(!item.hasClass(activeClass) && jQuery(attribute).length){
					items.removeClass(activeClass)
					item.addClass(activeClass);
					flag = true;
					offset = jQuery(attribute).offset().top - header.outerHeight() - subWrap.outerHeight();
					if(win.width() > mobileWidth){
						if(isWinPhoneDevice) jQuery(window).scrollTop(offset);
						else{
							jQuery('html, body').stop().animate({scrollTop: offset}, {duration: animSpeed, complete: function(){
								flag = false;
							}});
						}
					}
					else{
						if(isWinPhoneDevice) jQuery(window).scrollTop(jQuery(attribute).offset().top);
						else{
							jQuery('html, body').stop().animate({scrollTop: jQuery(attribute).offset().top}, {duration: animSpeed, complete: function(){
								flag = false;
							}});
						}
					}
			...
			if(section.length){
				jQuery(window).bind('scroll', function(){
					if(win.width() > mobileWidth){
						if(!flag && win.scrollTop() > section.offset().top - header.outerHeight() - subWrap.outerHeight() && win.scrollTop() < section.offset().top + section.height() - header.outerHeight() - subWrap.outerHeight()){
							items.removeClass(activeClass);
							item.addClass(activeClass);
						}
					}
	...
	function listPosition(){
		if(win.width() > mobileWidth){
			if(subWrap.closest('.section-subnav').length){
				if(win.scrollTop() + header.outerHeight() > navOffset) {
					subWrap.addClass(fixedClass).css({top: header.outerHeight()});
					subWrap.closest('.section-subnav').css({paddingBottom: subWrap.outerHeight()});
				}
				else{
					subWrap.removeClass(fixedClass).css({top: ''});
					navOffset = subWrap.offset().top;
					subWrap.closest('.section-subnav').css({paddingBottom: ''});
				}
			}
			else{
				var parent = subWrap.closest('section'),
...
//				else subWrap.removeClass(fixedClass).css({top: ''});
			...
			subWrap.removeClass(fixedClass).css({top: ''});
			if(subWrap.closest('.section-subnav').length) subWrap.closest('.section-subnav').css({paddingBottom: ''});
		}
	}
	listPosition();
	win.bind('scroll resize', listPosition);
	
	jQuery('.btn-smooth').each(function(){
		var btn = jQuery(this);
		var attrBtn = btn.attr('href');
		btn.bind(event, function(e){
			e.preventDefault();
			offset = jQuery(attrBtn).offset().top - header.outerHeight() - subWrap.children().outerHeight();
			if(win.width() > mobileWidth){
				if(isWinPhoneDevice) jQuery(window).scrollTop(offset);
				else{
					jQuery('html, body').stop().animate({scrollTop: offset}, {duration: animSpeed, complete: function(){
				...
				if(isWinPhoneDevice) jQuery(window).scrollTop(jQuery(attrBtn).offset().top);
				else{
					jQuery('html, body').stop().animate({scrollTop: jQuery(attrBtn).offset().top}, {duration: animSpeed, complete: function(){
						flag = false;
					}});
				}
			}
...
function initChildClasses() {
	jQuery('.pagination').children(':last-child').addClass('last-child');
	jQuery('.schedule-list').children(':nth-child(2n)').addClass('nth-child-2n');
}
/*
